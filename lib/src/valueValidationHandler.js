...
const valueValidationHandler = (dataTemplate, rawData) => {
    let validationTemplate = buildDictionaryFromArray("key", "validation", dataTemplate);
    let errors = [];
        ...
        rawDataNodeKeys.forEach(rawDataNodeKey => {
            ...
            if (valueRequired) {
                if (isNotEmptyString(val))
                    errors.push(ValidationMessage("valueRequired", index, rawDataNodeKey, val));
                switch (fieldType) {
                    case "email":
                        if (isEmailNotValid(val))
                            errors.push(ValidationMessage(fieldType, index, rawDataNodeKey, val));
                        break;
                    case "number":
                        if (isNaN(val))
                            errors.push(ValidationMessage(fieldType, index, rawDataNodeKey, val));
                    ...
                    case "boolean":
                        ...
                        break;
                    case "date":
                        if (!moment(val, validationTemplate[rawDataNodeKey]["format"]).isValid())
                            errors.push(ValidationMessage(fieldType, index, rawDataNodeKey, val));
                        break;
                }
            }
...
const isValidBoolean = val => val.toLowerCase() === "true"
    ? true
    : val.toLowerCase() === "false"
        ? true
        : false;
const isEmailNotValid = val => val.split("@").length - 1 < 1;
const ValidationMessage = (validationType, nodeIndex, onKey, withValueOf) => `${validationType} check failed on node ${nodeIndex}: ${onKey} value ${withValueOf}`;
